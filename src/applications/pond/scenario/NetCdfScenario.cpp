      ...
      NetCdfData bathyData(bathymetryFilename);
          ...
          for (int i = 0; i < upcxx::rank_n(); i++) {
              upcxx::rpc(
                  ...
                  [](upcxx::view<float> bathyView, SimulationArea s, size_t
                  bxs, size_t bys, upcxx::dist_object<NetCdfScenario *>
                  &rThis) {
                      std::vector<float> &rBathymetryData =
                      (*rThis)->bathymetryData; std::copy(bathyView.begin(),
                      bathyView.end(),  back_inserter(rBathymetryData));
